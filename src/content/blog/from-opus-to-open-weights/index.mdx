...
- Zero cache-read/cache-write tokens recorded. This is a clear opportunity to further reduce cost. Prompt caching on the input side (the dominant cost driver) could cut spend further if/when supported.
...
I've now gone further and integrated the same endpoint into Solar Dashboard as one of the options for the backend of the AI Assistant. Here's an example of it in action providing insight on peak solar generation months:

![Solar Dashboard AI Assistant answering "What are the top 5 highest generating months of all time and what is the trend?" with a ranked table and bar chart, topped by July 2026 at 679.0 kWh](./images/solar-dashboard-assistant.png)
